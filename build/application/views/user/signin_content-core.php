      ...
      <div class="flex flex-column items-center z1 h-100">
        ...
        <div id="widget-signin" class="widget rounded h-100">
          <form id="signInForm">
            ...
            <a class="block mb-1 h5 primary hyper-link big-modal" href="<?= base_url(LANG('SETT_LINK_RECOVER_ACCESS')); ?>">
							<?= lang('USER_SIGNIN_ACCESS_RECOVER'); ?>
						</a>
            <?php endif; ?>
            <?php if (lang('SETT_SIGIN') == 'ON') : ?>
            <p class="mb-0 h5 center"><?= lang('USER_SIGNIN_NO_USER') ?>
              <a class="hyper-link big-modal" href="<?= base_url(lang('SETT_LINK_USER_IDENTIFY')) ?>"><?= lang('USER_SIGNIN_SINGN_UP') ?></a>
            </p>
            <?php endif; ?>
          </form>
        </div>
