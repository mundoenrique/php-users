...
<div id="recovery" class="recovery-content h-content hide-out hide">
	<div class="max-width-4 mx-auto p-responsive py-4">
		...
		<form id="channelForm">
			...
			<?php if($channel === lang('SETT_MFA_CHANNEL_APP')): ?>
				<div>
					<p><?= lang('MFA_TWO_FACTOR_CODE_APP'); ?>
					<?php foreach (lang('SETT_LINK_MFA_APPS') as $key => $value) : ?>
						<a href="<?= lang('SETT_LINK_MFA_APPS')[$key]; ?>" class="btn-link " target="_blank">
							<?= $key; ?>,
						</a>
					<?php endforeach; ?>
					etc.
				...
				<div>
					...
					<a id="resendCode" href=<?= lang('SETT_NO_LINK') ?> class="btn btn-small btn-link p-0 big-modal" ><?= lang('GEN_BTN_RESEND_CODE'); ?></a>
				</p>
				</div>
			<?php endif; ?>
			...
			<div class="max-width-1 fit-lg mx-auto">
				<form id="mfaConfirmForm" class="mt-2" method="POST">
					...
					<div class="flex items-center justify-end pt-3">
						<a class="btn btn-small btn-link big-modal" href="<?= base_url(lang('SETT_LINK_MFA_ENABLE')); ?>"><?= lang('GEN_BTN_CANCEL'); ?></a>
