...
$(function () {
	...
	function generar_info(tarjeta, tipo, producto, idpersona, fechaIni, fechaFin, consulta) {
		...
		$.post(base_url + "/report/CallWsGastos", { request: dataRequest, cpo_name: cpo_cook, plot: btoa(cpo_cook) }, function (response) {
			...
			result += '<p>Vuelva a realizar la busqueda con un filtro distinto para obtener resultados.</p>';
			...
			if (data.rc != 0) {
				...
				if (data.rc == -9999) {
					$('#empty-state.nodata-state > h2').text('Atención');
					$('#empty-state.nodata-state > p').text('Combinación de caracteres no válida.');
				}
				if (data.rc == -150) {
					$("#report-detail").children().remove();
					$("#chart").children().remove();
