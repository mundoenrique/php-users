...
function buildTransferResultModal() {
  var setObjectResult, objectResult, resultValueObject, msgText;
  ...
  resultValueObject = {
    ...
    msg: lang.TRANSF_TRANSFER_EXTERNAL_BANK
  };
  ...
  Object.entries(objectResult).forEach(([name, text]) => {
    ...
    if (operationType == "PCI") {
      msgText = (text == 'msg') ? resultValue : text + ": " + resultValue;
    } else {
      msgText = text + ": " + resultValue;
    }
    span = $("<span></span>")
      .addClass("list-inline-item")
      .text(msgText);
