...
// ----------------------------------------------------------------------------------------------------------------------------------------------------------------
	...
	function buscar_historial(ctaOrigen,mes,anio){
		...
		$.post(base_url +"/historial/historial",{"noTarjeta":ctaOrigen,"tipoOperacion":"P2C","mes":mes,"anio":anio},function(data){
			if(data.rc == -61){
			...
			if(data.rc==0){
				...
				$.each(data.listaTransferenciasRealizadas,function(pos,item){
	            	...
	            	var lista=			'<li class="feed-item '+clase1+'">';
						...
						lista+=				'</div>';
						lista+=				tipo + ': ' +item.beneficiario+'<span class="money-amount"> '+moneda+' '+item.montoTransferencia+' </span>';
...
// ----------------------------------------------------------------------------------------------------------------------------------------------------------------
	...
	function convert_mes($mes){
		switch (mes)
		{
		case 1:
		 mes="Today it's Sunday";
		  break;
		case 1:
		  x="Today it's Monday";
		  break;
