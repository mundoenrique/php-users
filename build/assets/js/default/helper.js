...
function callNovoCore(verb, who, where, data, _response_) {
    ...
    var cpo_cook = decodeURIComponent(document.cookie.replace(/(?:(?:^|.*;\s*)cpo_cook\s*\=\s*([^;]*).*$)|^.*$/, '$1'));

    var dataRequest = JSON.stringify({who: who, where: where, data: data});
...
function notiSystem(title, message, icon, data) {
    ...
    dialogMoldal.dialog({
        ...
        open: function (event, ui) {
            ...
            $('#accept').text(btn1.text).on('click', function (e) {
                if (btn1.action !== 'wait') {
                    dialogMoldal.dialog('close');
                    $(this).off('click');
                }
            ...
            $('#cancel').hide();
            if (btn2) {
                $('#cancel').text(btn2.text).on('click', function (e) {
                    dialogMoldal.dialog('close');
...
function createFields(fields) {
    ...
    for (var field of fields) {
        switch (field.typeElement) {
            ...
            case 'password': element = $(`<input id="${
                    field.id
